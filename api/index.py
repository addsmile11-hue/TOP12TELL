...
def parse_naver_sise(url):
    """네이버 금융 시세 페이지 파싱 및 원천 데이터 날짜 추출"""
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
    try:
        res = requests.get(url, headers=headers, timeout=5)
        res.encoding = 'euc-kr'
        
        # 📅 단순함 속의 해답: 페이지 소스 내에서 '2026.05.15 16:00' 형태의 공식 마켓 시계만 정확히 가로챕니다.
        market_date = None
        date_match = re.search(r'(\d{4})\.(\d{2})\.(\d{2})\s+\d{2}:\d{2}', res.text)
        if date_match:
...
def get_stock_fundamentals(ticker):
    """개별 종목 상세 지표 파싱 (날짜 로직을 완전히 제거하여 복잡성 제거)"""
    ...
    try:
        ...
        if market_sum_elem:
            ...
            if not market_sum.endswith('억'): market_sum += '억'
        else:
            market_sum = "N/A"
...
def get_stock_data():
    ...
    kd_cap_stocks, kd_date = results["kd_cap"]
    k_val_stocks, kv_date = results["k_val"]
    kd_val_stocks, kvd_date = results["kd_val"]
    
    # 가자 먼저 매칭이 성공한 깨끗한 마켓 원천 거래일 채택
    final_date = market_date or kd_date or kv_date or kvd_date
    if not final_date:
        final_date = datetime.datetime.now().strftime("%Y년 %m월 %d일")
